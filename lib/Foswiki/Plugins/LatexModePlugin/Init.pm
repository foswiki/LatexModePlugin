...
sub doInit {
    ...
    $LMPcontext{'rerender'} = &Foswiki::Func::getPreferencesValue("RERENDER")
      || 0;
    if ( ($query) and $query->param('latex') ) {
        $LMPcontext{'rerender'} = ( $query->param('latex') eq 'rerender' );
    }

    $LMPcontext{'alltexmode'} =
      &Foswiki::Func::getPreferencesValue("LATEXMODEPLUGIN_ALLTEXMODE") || 0;
    if ( ($query) and $query->param('latex') ) {
    ...
    &Foswiki::Func::writeDebug(
        "- Foswiki::Plugins::LatexModePlugin::doInit() is OK")
      if $debug;
