...
my $GREP =
     $Foswiki::cfg{Plugins}{LatexModePlugin}{fgrep}
  || $Foswiki::fgrepCmd
  || '/usr/bin/fgrep';
...
my $convertargs =
  " -density %DENSITY|N%" . "  %EPS|F% -antialias -trim -gamma %GAMMA|N% ";
...
### my ( $math_string ) = @_;   # do not uncomment, use $_[0], $_[1] instead
    ...
    if ( ( $LMPc{'use_color'} == 1 )
        and !( $LMPc{'preamble'} =~ m/ackage\{color/ ) )
    {
        $LMPc{'preamble'} = "\\RequirePackage{color}\n" . $LMPc{'preamble'};
    }
    if ( ( $LMPc{'preamble'} =~ m/package\{color/i )
        and !( $LMPc{'preamble'} =~ m/definecolor\{Red/ ) )
    {
...
COLORS
    ...
    if ( exists( Foswiki::Func::getContext()->{'genpdflatex'} ) ) {
        ...
        my $image_name = join(
            '/',
            (
                &Foswiki::Func::getPubUrlPath(), $LMPc{'web'},
                $LMPc{'topic'},                  "latex$hash_code.$EXT"
            )
        );
        ...
        if (   ( $opts{'inline'} eq 1 )
            or ( $opts{'inline'} eq "on" )
            or ( $opts{'inline'} eq "true" ) )
        {
            ...
            if ($tweakinline) {
                ...
                $algn =
                  ( $escaped =~ m/[\_\}\{]|[yjgpq]/ ) ? 'middle' : 'bottom';
            }
            $txt =
"<img style=\"vertical-align:$algn;\" align=\"$algn\" $str src=\"$image_name\" alt=\"$escaped\" />";
...
"&nbsp;&nbsp;&nbsp;Error! multiple equation labels '$opts{'label'}' defined.\n"
            ...
            $txt =
"<div align=\"center\"><img src=\"$image_name\" $str alt=\"$escaped\" /></div>";
        }
...
sub createTempLatexFiles {
    ...
    do {
        $_[0] .= "<BR>can't write $LATEXFILENAME: $!\n";
        return;
    } unless open( DVIOUT, ">$LATEXFILENAME" );

    my $PDFLATEXFILENAME = 'pdf_' . $LATEXFILENAME;
    do {
        $_[0] .= "<BR>can't write $PDFLATEXFILENAME: $!\n";
        return;
    } unless open( PDFOUT, ">$PDFLATEXFILENAME" );
    ...
    if ( Foswiki::Func::getContext()->{'LMPcontext'}->{'docclass'} ) {
        $txt .=
          Foswiki::Func::getContext()->{'LMPcontext'}->{'docclass'} . "\n";
    }
    else {
...
# my %opts = defined($LMPc{'markup_opts'}{$key}) ? %{$LMPc{'markup_opts'}{$key}} : ();
        ...
        if ( exists( $opts{'attachment'} ) ) {
                ...
                or ($bypassattach) )
            {
                ...
                $af = join( $pathSep,
                    &Foswiki::Func::getPubDir(),
                    $LMPc{'web'}, $LMPc{'topic'}, $opts{'attachment'} );
                ...
                foreach my $e (@extlist) {
                    ...
                    if ( -f $af . $ext ) {
                        &Foswiki::Func::writeDebug(
                            "LatexModePlugin: copy " . $af . $ext )
                          if ($debug);

                        # copy( $af.$ext, $LATEXWDIR ) || do {
                        copy( $af . $ext, "." ) || do {
                            &Foswiki::Func::writeDebug(
                                "LatexModePlugin: copy failed " . $! );
                            $value =
                                "attachment \{"
                              . $markup_opts{$key}->{'attachment'}
                              . "\} \ not found";
                        };
        ...
        $value =
          " (attachment " . $markup_opts{$key}->{'attachment'} . " not found) "
          if ( exists( $markup_opts{$key}->{'attachment'} )
            and !( -f $markup_opts{$key}->{'attachment'} ) );
        ...
        if ( $markup_opts{$key}->{'engine'} eq 'pdf' ) {
            ...
            do {
                $_[0] .= "<BR>can't write $MIMELATEXFILENAME: $!\n";
                return;
            } unless open( MIMEOUT, ">$MIMELATEXFILENAME" );
...
sub renderEquations {
 ...
 # print STDERR '-'x70; print STDERR "\n";
    ...
    &Foswiki::Func::writeDebug( " Foswiki::LatexModePlugin::renderEquations( "
          . $LMPc{'web'} . '.'
          . $LMPc{'topic'}
          . " )" )
      if $debug;
    ...
    if ( -x $PATHTOLATEX ) {
        ( $resp, $exit ) =
          Foswiki::Sandbox->sysCommand( "$PATHTOLATEX " . ' --help' );
    }
    elsif ( -x $PATHTOPDFLATEX ) {
        ( $resp, $exit ) =
          Foswiki::Sandbox->sysCommand( "$PATHTOPDFLATEX " . ' --help' );
    }
...
# &Foswiki::Func::writeDebug( join(" ", keys(%hashed_math_strings) ) ) if ($debug);
    ...
    $_[0] .=
      ...
      . $LMPc{'error_catch_all'}
      if ( length( $LMPc{'error_catch_all'} ) > 0 );
    ...
    my %extfiles = ();
    $path =
      &Foswiki::Func::getPubDir() . "/" . $LMPc{'web'} . '/' . $LMPc{'topic'};
    if (   ( $Foswiki::Plugins::VERSION < 1.1 )
        or ($bypassattach) )
    {
        ...
        # database interface
        my ( $meta, undef ) =
          Foswiki::Func::readTopic( $LMPc{'web'}, $LMPc{'topic'} );
    ...
    &Foswiki::Func::writeDebug(
        "LatexModePlugin::Render - Scanning file attachments")
      if $debug;
    &Foswiki::Func::writeDebug(
        " pre-hashes: " . join( ' ', keys(%hashed_math_strings) ) )
      if $debug;
    ...
    foreach my $a ( keys %extfiles ) {
        ...
        if ( $fn =~ m/^latex[0-9a-f]+\.$EXT$/ ) {
              ...
              )
            {
                ...
                &Foswiki::Func::writeDebug(
                    "Deleting old image that I think belongs to me: $fn "
                      . ( -f $path . $pathSep . $fn ) )
                  if $debug;
                if ( $fn =~ /^([-\@\w.]+)$/ ) {               # untaint filename
                        ...
                        or ($bypassattach) )
                    {
                        ...
                        Foswiki::Func::moveAttachment( $LMPc{'web'},
                            $LMPc{'topic'}, $fn, $Foswiki::cfg{TrashWebName},
                            'TrashAttachment', $fn )
                          if ( -f $path . $pathSep . $fn )
                          ;    # should be replaced by attacmentexists? function
                    }
                }
            }
        }
    ...
    &Foswiki::Func::writeDebug(
        " post-hashes: " . join( ' ', keys(%hashed_math_strings) ) )
      if $debug;
    ...
    foreach my $key ( keys %hashed_math_strings ) {
            ...
            or ($bypassattach) )
        {
            ...
            # database interface
            if (
                Foswiki::Func::attachmentExists( $w, $t,
                    'latex' . $key . '.' . $EXT )
                and !( $LMPc{'rerender'} )
              )
            {
    ...
    &Foswiki::Func::writeDebug("LatexModePlugin working directory: $LATEXWDIR")
      if $debug;
    ...
    if ( -e $LATEXWDIR ) {
        ...
        &Foswiki::Func::writeDebug(" Directory $LATEXWDIR does not exist")
          if $debug;
    }
    ...
    do {
        $_[0] .= "<BR>unable to access latex working directory.";
        return;
    } unless chdir($LATEXWDIR);
...
# system("$PATHTOLATEX -interaction=nonstopmode -halt-on-error $LATEXFILENAME >> $LATEXLOG 2>&1");
    if ( scalar(%pdf_hash_code_mapping) ) {
        ...
        my ( $resp, $ret ) = Foswiki::Sandbox->sysCommand(
            $GREP . ' -A 2 Error %LOG|F%',
    ...
    if ( scalar(%dvi_hash_code_mapping) ) {
            ...
            || ( Foswiki::Func::getContext()->{'save'} ) )
        {
            ...
            my ( $resp, $ret ) =
              Foswiki::Sandbox->sysCommand( $GREP . ' -A 1 ! %LOG|F%',
    ...
    if (%dvi_hash_code_mapping) {
        if ( -f $LATEXBASENAME . ".dvi" ) {
            &makePNGs( \%dvi_hash_code_mapping, $LMPc{'topic'}, $LMPc{'web'},
                $LATEXLOG, $LATEXWDIR );
        }
        else {
            $_[0] .=
              "<br>Latex rendering error!! dvi file was not created.<br>";
            $_[0] .= "<pre>";
    ...
    if (%mimetex_hash_code_mapping) {
        &Foswiki::Func::writeDebug(
            "mimetex: " . join( ' ', keys %mimetex_hash_code_mapping ) )
          if $debug;

        &makePNGs( \%mimetex_hash_code_mapping, $LMPc{'topic'}, $LMPc{'web'},
            $LATEXLOG, $LATEXWDIR );

    }
    if (%pdf_hash_code_mapping) {
        if ( -f 'pdf_' . $LATEXBASENAME . ".pdf" ) {
            &makePNGs( \%pdf_hash_code_mapping, $LMPc{'topic'}, $LMPc{'web'},
                $LATEXLOG, $LATEXWDIR );
        }
        else {
            $_[0] .=
              "<br>Latex rendering error!! pdf file was not created.<br>";
    ...
    unless ($debug) {
        ...
        while ( ( my $key, my $value ) = each(%hashed_math_strings) ) {
            ...
            if ( exists( $opts{'attachment'} ) ) {
                ...
                my $af = join( '/', $LATEXWDIR, $opts{'attachment'} );
                unlink($af);
            }
        ...
        foreach my $fn (@files) {
            ...
            if ( $fn =~ /^([-\@\w.]+)$/ ) {
                ...
                &Foswiki::Func::writeDebug(
                    "Bizzare error.  match of \$fn failed? $fn")
                  if $debug;
            }
        }
    }
...
sub makePNGs {
    ...
    while ( ( my $key, my $value ) = each(%hash_code_mapping) ) {
        ...
        system(
            "echo \"engine: $opts{'attachment'} $opts{'engine'}\" >> $LATEXLOG")
          if ( exists( $opts{'attachment'} ) and ($debug) );
        if (    ( -x $PATHTODVIPNG )
            and ( $opts{'engine'} ne 'ps' )
            and ( $opts{'engine'} ne 'mimetex' )
            and ( $opts{'engine'} ne 'pdf' ) )
        {
          ...
          # system($cmd);
            ...
            $ccmd .= " -transparent %BGC|S% "
              unless ( ( $markup_opts{$key}{'inline'} ne 0 )
                and ( $tweakinline ne 0 ) );
            $ccmd .= " %OUTIMG|F%";
            ...
            my ( $data, $ret ) =
              Foswiki::Sandbox->sysCommand( $ccmd, IN => $key . '.txt' )
              if ( -x $PATHTOMIMETEX );
...
# system("$PATHTODVIPS -E -pp $num -o $LATEXBASENAME.$num.eps $LATEXBASENAME.dvi >> $LATEXLOG 2>&1 ");
            ...
            # $cmd .= $outimg;
            #
            my ( $d, $e ) = Foswiki::Sandbox->sysCommand(
                "$PATHTODVIPS $dvipsargs",
                NUM => $num,
        ...
        if ( -f $outimg ) {
            ...
            if (    ( $markup_opts{$key}{'inline'} ne 0 )
                ...
                and ( -x $PATHTOCONVERT ) )
            {
                trimInlineImage( $outimg, '', $opts{'bgcolor'}, $LATEXWDIR,
                    $ptsz );
            }
                ...
                || ($bypassattach) )
            {
                ...
                move( $outimg, $path . $pathSep . $outimg )
                  or $_[0] .=
                  "<br> LatexModePlugin error: Move of $outimg failedg: $!";
            }
            else {
...
sub trimInlineImage_v2 {
    ...
    Foswiki::Sandbox->sysCommand(
        "$PATHTOCONVERT -colors 16 -trim %IN|F%  %OUT|F%",
        IN  => $in,
        OUT => $out
    ) if ( -x $PATHTOCONVERT );
    ...
    while (<F>) {
        ...
        if ($flag) {
            ...
            $midu = $cnt
              if ( !( substr( $_, 1, $cpp ) =~ m/\Q$canvaschar\E|\;/ )
                and ( $midu eq 0 ) );
    ...
    $back = length($1) * $cpp
      if ( $sum =~ m/(0+1+0{0,2})$/ );    # and from the right
                                          # print "frnt: $frnt back: $back\n";

    $pre =~ m/\"$col\s+$cnt/;
    my $nr = $cnt + $adt + $adb;          # calculate the new number of rows
    my $nc =
      $col - ( $frnt + $back ) / $cpp;    # calculate the new number of columns
    ...
    Foswiki::Sandbox->sysCommand(
        "$PATHTOCONVERT mod_%OUT|F% -antialias -transparent %BGC|S% %IN|F%",
        OUT => $out,
        BGC => $bgcolor,
        IN  => $in
    ) if ( -x $PATHTOCONVERT );
...
sub trimInlineImage_v1 {
    ...
    my ( $d, $e ) = Foswiki::Sandbox->sysCommand(
        "$PATHTOCONVERT %IN|F% " . " -background %BGC|S% -trim " . " %OUT|F% ",
        IN  => $tmpfile,
        BGC => $bgcolor,
        OUT => $outimg
    ) if ( -x $PATHTOCONVERT );
    ...
    ( $d, $e ) = Foswiki::Sandbox->sysCommand(
        "$PATHTOCONVERT %INIMG|F% "
          . " -crop "
          . '%NW|N%' . 'x'
          . '%NH|N%' . '+'
          . '%SH|N%' . '+'
